...
import { AlertCircle, BookOpen, GraduationCap, Shield, Trophy, Users, Zap } from 'lucide-react';
import { useState } from 'react';
import FloatingHelpButton from '../components/ui/FloatingHelpButton';
import GuideTooltip, { useGuide } from '../components/ui/GuideTooltip';
import { getUser, User } from '../data/database';
...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simular un pequeño delay para el acceso
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    try {
      // Buscar usuario por código o CI si se proporciona
      let user = null;
      if (formData.code) {
        user = getUser(formData.code);
      }
      
      if (!user) {
        // Si no se encuentra por código, crear usuario temporal con los datos ingresados
        // En una implementación real, aquí se registraría en la base de datos
        const newUser: User = {
          id: `temp-${Date.now()}`,
          name: formData.name,
          ci: formData.ci,
          ...
          code: formData.code || `TEMP${Date.now()}`,
          role: selectedRole,
          ...
          profile: selectedRole === 'student' ? {
            ...
            ranking: { course: 0, school: 0, country: 0 },
            ownedFrames: ['none'],
            ownedBanners: ['cosmic'],
            selectedFrame: 'none',
      ...
      setLoginError('Error al acceder. Inténtalo de nuevo.');
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center px-4">
        ...
        >
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-gray-400 text-sm">Gestiona clases y evalúa estudiantes</p>
                  </div>
                </div>
              </motion.button>
            </motion.div>
          ...
          {/* Login Form simplificado y realista */}
          {selectedRole && (
            <motion.form
            ...
            >
              ...
              <div className="space-y-4">
                ...
                >
                  <label className="block text-sm font-medium mb-2 font-orbitron tracking-wider text-gray-300">
                    Nombre Completo
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full p-3 bg-gray-950/70 border border-gray-700/50 rounded-xl focus:border-blue-500 focus:outline-none transition-all font-jetbrains text-gray-100 focus:bg-gray-950/90"
                ...
                >
                  <label className="block text-sm font-medium mb-2 font-orbitron tracking-wider text-gray-300">
                    {selectedRole === 'student' ? 'Grado y Sección' : 'Área/Materia'}
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full p-3 bg-gray-950/70 border border-gray-700/50 rounded-xl focus:border-blue-500 focus:outline-none transition-all font-jetbrains text-gray-100 focus:bg-gray-950/90"
                    placeholder={selectedRole === 'student' ? "Ej: 7mo A" : "Ej: Matemáticas"}
                    required
                    disabled={isLoading}
                  />
                </motion.div>
                ...
                >
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-gray-950/70 border border-gray-700/50 rounded-xl focus:border-blue-500 focus:outline-none transition-all font-jetbrains text-gray-100 focus:bg-gray-950/90"
                    placeholder={selectedRole === 'student' ? "Ej: EST240001" : "Ej: DOC240001"}
                    disabled={isLoading}
                  />
                </motion.div>
              </div>
              ...
              >
                <h4 className="text-sm font-medium text-blue-300 mb-2 font-orbitron">Usuarios de Ejemplo:</h4>
                <div className="space-y-2 text-xs font-rajdhani">
                  {selectedRole === 'student' ? (
                    <>
                      <div className="text-gray-300">
                        <span className="text-blue-400">Nombre:</span> Alex Rivera | 
                        <span className="text-blue-400"> CI:</span> 12345678 | 
                        <span className="text-blue-400"> Código:</span> EST250001
                      </div>
                      <div className="text-gray-300">
                        <span className="text-blue-400">Nombre:</span> María López | 
                        <span className="text-blue-400"> CI:</span> 87654321 | 
                        <span className="text-blue-400"> Código:</span> EST250002
                      </div>
                    </>
                  ) : (
                    <>
                      <div className="text-gray-300">
                        <span className="text-purple-400">Nombre:</span> Prof. Luis García | 
                        <span className="text-purple-400"> CI:</span> 98765432 | 
                        <span className="text-purple-400"> Código:</span> DOC250001
                      </div>
                      <div className="text-gray-300">
                        <span className="text-purple-400">Nombre:</span> Prof. Ana Martínez | 
                        <span className="text-purple-400"> CI:</span> 55667788 | 
                        <span className="text-purple-400"> Código:</span> DOC250002
                      </div>
                    </>
                  )}
                </div>
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    <span className="font-jetbrains">&gt;&gt;</span> ACCEDER <span className="font-jetbrains">&lt;&lt;</span>
                  </>
                )}
              </motion.button>
            </motion.form>
